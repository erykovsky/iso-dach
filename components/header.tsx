   ...
   <div className="container mx-auto px-4 py-4 flex justify-between items-center">
    ...
    <nav className="hidden lg:flex items-center space-x-6">
     ...
     >
      ...
      <div
       className={`absolute left-0 top-[calc(100%+2px)] w-72 bg-white rounded-md shadow-lg py-2 z-20 transition-all duration-300
                ${
      ...
      >
       {services.map((service, index) => (
        <Link
         ...
         href={service.href}
         prefetch={true}
         className="block px-4 py-3 text-gray-800 hover:bg-gray-100 hover:text-primary transition-colors w-full"
     ...
     <Link
      href="/galeria"
      prefetch={true}
      className="text-primary hover:text-secondary transition duration-300 ease-in-out px-3 py-2 block"
     >
      Galeria
     </Link>
     <Link
    ...
    <div className="lg:hidden absolute top-full left-0 right-0 bg-white shadow-md">
     <nav className="flex flex-col items-center py-4 space-y-1">
      ...
      <div className="w-full text-center py-2">
       <button
        className="inline-flex items-center text-primary hover:text-secondary transition duration-300 ease-in-out"
       ...
       {isMobileServicesOpen && (
        <div className="mt-1 w-full flex flex-col items-center space-y-1 py-1 bg-gray-50">
         {services.map((service, index) => (
          <Link
           key={index}
           href={service.href}
           prefetch={true}
           className="text-gray-700 hover:text-primary transition-colors py-2 px-4 block w-full text-center"
           onClick={() => {
            setIsMobileServicesOpen(false);
            setIsMenuOpen(false);
           }}
      ...
      <Link
       href="/galeria"
       prefetch={true}
       className="text-primary hover:text-secondary transition duration-300 ease-in-out px-4 py-2 w-full text-center block"
       onClick={toggleMenu}
      >
       Galeria
      </Link>
      ...
      <div className="flex items-center space-x-4 py-2 w-full justify-center">
       <a
        href="https://facebook.com/iso-dach-1639181143011116"
        target="_blank"
        rel="noopener noreferrer"
      ...
      <a
       href="tel:+48660441941"
       className="flex items-center justify-center space-x-2 text-primary hover:text-secondary transition duration-300 ease-in-out py-2 w-full"
      >
       <Phone size={20} />
       <span>+48 660 441 941</span>
      </a>
